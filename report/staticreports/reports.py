...
def getPitstopTime():
    with connection.cursor() as cursor:
        cursor.execute('SELECT r.year, c.circuitId, c.name, \
                            MIN(p.milliseconds) AS MinTime, \
                            FLOOR(AVG(p.milliseconds)) AS AvgTime \
                        FROM f1db.pitStops p \
                        INNER JOIN f1db.races r ON r.raceId = p.raceId \
                        INNER JOIN f1db.circuits c ON c.circuitId = r.circuitId AND c.circuitId IN (1, 4, 6, 9, 14) \
                        GROUP BY r.year, c.circuitId, c.name \
                        ORDER BY r.year, c.circuitId, c.name')
        report = cursor.fetchall()
    return report
